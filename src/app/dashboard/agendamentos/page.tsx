...
export default function Agendamentos() {
  ...
  const filtros = [
    { label: t('agendamentos') + ' - Todos', value: 'TODOS' },
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 24, flexWrap: 'wrap' }}>
        {[
          ...
          <button key={f.value} className="filtro-btn" onClick={() => setFiltro(f.value)}
            ...
            {f.label}
            <span style={{ marginLeft: 6, fontSize: 11, opacity: .8 }}>({f.value === 'TODOS' ? agendamentos.length : agendamentos.filter(a => a.status === f.value).length})</span>
          </button>
        ))}
      </div>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 12 }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ display: 'flex', gap: 16, marginTop: 8, flexWrap: 'wrap', alignItems: 'center' }}>
                      ...
                      {pgto && <span style={{ fontSize: 12, fontWeight: 600, background: darkMode ? '#2a2a2a' : '#f3f4f6', color: darkMode ? '#ccc' : '#374151', padding: '3px 10px', borderRadius: 100, display: 'flex', alignItems: 'center', gap: 4 }}>{pgto.emoji} {pgto.label}</span>}
                    </div>
                  ...
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
                    ...
                    {a.status === 'CONFIRMADO' && (
                      <button onClick={() => atualizarStatus(a.id, 'CONCLUIDO')} style={{ background: '#6366f1', color: '#fff', padding: '8px 14px', borderRadius: 100, fontSize: 12, fontWeight: 600, cursor: 'pointer', border: 'none' }}>✓ Concluir</button>
                    )}
                    <button className="excluir-btn" onClick={() => excluir(a.id, a.status)} style={{ background: bg2, border: '1.5px solid #fecaca', color: '#dc2626', padding: '8px 14px', borderRadius: 100, fontSize: 12, cursor: 'pointer', transition: 'all .15s' }}>🗑 Excluir</button>
                  </div>
                </div>
              </div>
            )
