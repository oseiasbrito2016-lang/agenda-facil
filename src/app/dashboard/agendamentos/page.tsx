  ...
  async function excluir(id: string, status: string) {
    const confirmado = window.confirm(
      status === 'CONFIRMADO' || status === 'CONCLUIDO'
        ? 'Este agendamento será excluído e o valor será descontado do faturamento. Confirma?'
  ...
  return (
    <main style={{ minHeight: '100vh', fontFamily: "'DM Sans', sans-serif", padding: '40px 32px' }}>
      ...
      <div style={{ marginBottom: 32 }}>
        <h1 style={{ fontFamily: "'Instrument Serif', serif", fontSize: 32, color: '#0A0A0A' }}>Agendamentos</h1>
        <p style={{ color: '#666', fontSize: 14, marginTop: 4 }}>Gerencie todos os agendamentos do seu estabelecimento</p>
      </div>

      {/* Filtros */}
      <div style={{ display: 'flex', gap: 8, marginBottom: 24, flexWrap: 'wrap' }}>
        {filtros.map(f => (
          <button key={f.value} className="filtro-btn"
            onClick={() => setFiltro(f.value)}
            style={{
              padding: '8px 16px', borderRadius: 100, fontSize: 13, fontWeight: 500,
              border: '1.5px solid ' + (filtro === f.value ? '#00C27C' : '#eee'),
              background: filtro === f.value ? '#00C27C' : '#fff',
      ...
      {carregando ? (
        ...
        <div style={{ textAlign: 'center', padding: 60, background: '#fff', borderRadius: 16, border: '1px solid #eee' }}>
          ...
          <h3 style={{ fontSize: 18, fontWeight: 600, color: '#0A0A0A', marginBottom: 8 }}>Nenhum agendamento</h3>
          <p style={{ fontSize: 14, color: '#666' }}>Compartilhe seu link para começar a receber agendamentos.</p>
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {filtrados.map(a => {
            ...
            return (
              <div key={a.id} style={{ background: '#fff', borderRadius: 12, padding: '20px 24px', border: '1px solid #eee' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 12 }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
                      <div style={{ fontSize: 16, fontWeight: 600, color: '#0A0A0A' }}>{a.cliente?.nome}</div>
                    ...
                    <div style={{ fontSize: 13, color: '#666' }}>{a.cliente?.email}</div>
                    <div style={{ display: 'flex', gap: 16, marginTop: 8, flexWrap: 'wrap', alignItems: 'center' }}>
                      <span style={{ fontSize: 13, color: '#333' }}>✂️ {a.servico?.nome}</span>
                      {a.profissional && <span style={{ fontSize: 13, color: '#333' }}>👤 {a.profissional?.nome}</span>}
                      <span style={{ fontSize: 13, color: '#333' }}>📅 {formatarDataBR(a.data)} às {formatarHoraBR(a.data)}</span>
                    ...
                    {a.observacao && (
                      <div style={{ marginTop: 8, fontSize: 13, color: '#888' }}>💬 {a.observacao}</div>
                    )}
                  </div>
                  ...
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
                    ...
                    <button className="excluir-btn" onClick={() => excluir(a.id, a.status)}
                      style={{ background: '#fff', border: '1.5px solid #fecaca', color: '#dc2626', padding: '8px 14px', borderRadius: 100, fontSize: 12, cursor: 'pointer', transition: 'all .15s' }}>
                      🗑 Excluir
                    </button>
                  </div>
                </div>
