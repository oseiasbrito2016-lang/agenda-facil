...
function DashboardContent() {
  ...
  if (!usuario) return <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: bg }}><p style={{ color: text2 }}>Carregando...</p></div>
  ...
  return (
    <main style={{ minHeight: '100vh', background: bg, fontFamily: "'DM Sans', sans-serif", padding: '40px 32px', transition: 'background .2s' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            ...
            <div>
              <div style={{ fontSize: 16, fontWeight: 600, color: '#fff' }}>
                {agendamentosRestantes === 0 ? 'Você atingiu o limite do plano gratuito' : `Você tem ${agendamentosRestantes} agendamento(s) gratuito(s) restantes`}
              </div>
              <div style={{ fontSize: 13, color: '#888', marginTop: 4 }}>Assine o plano Pro por <strong style={{ color: '#00C27C' }}>R$49,90/mês</strong> e tenha agendamentos ilimitados</div>
            </div>
          </div>
          <button onClick={assinar} disabled={assinando} style={{ background: '#00C27C', color: '#fff', padding: '12px 28px', borderRadius: 100, fontSize: 14, fontWeight: 600, cursor: assinando ? 'not-allowed' : 'pointer', border: 'none', opacity: assinando ? 0.7 : 1, whiteSpace: 'nowrap' }}>
      ...
      <div style={{ marginBottom: 32 }}>
        ...
        <p style={{ color: text2, fontSize: 15, marginTop: 6 }}>Aqui está um resumo do seu estabelecimento hoje.</p>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 32 }}>
        {[
          { label: 'Agendamentos hoje', value: stats.agendamentosHoje, color: text, icon: '📅', iconBg: '#3b82f615' },
          { label: 'Clientes cadastrados', value: stats.clientes, color: text, icon: '👥', iconBg: '#ec489915' },
          { label: 'Faturamento do mês', value: formatarPreco(stats.faturamento), color: '#00C27C', icon: '💰', iconBg: '#00C27C15' },
        ].map((s, i) => (
      ...
      {estabelecimento && (
        <div style={{ background: '#0A0A0A', borderRadius: 16, padding: '20px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            <button onClick={() => { navigator.clipboard.writeText(window.location.origin + '/agendar/' + estabelecimento.slug); alert('Link copiado!') }} style={{ background: '#fff', color: '#0A0A0A', padding: '10px 20px', borderRadius: 100, fontSize: 13, fontWeight: 600, cursor: 'pointer', border: 'none' }}>📋 Copiar</button>
            <a href={'/agendar/' + estabelecimento.slug} target="_blank" style={{ background: '#00C27C', color: '#fff', padding: '10px 20px', borderRadius: 100, fontSize: 13, fontWeight: 600, textDecoration: 'none', display: 'inline-block' }}>👁 Ver página</a>
          </div>
        </div>
      )}
    </main>
